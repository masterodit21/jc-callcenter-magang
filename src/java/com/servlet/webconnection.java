 ...
 * @author user
 */
public class webconnection extends NikitaServlet{
    ...
    public void OnCreate(NikitaRequest request, NikitaResponse response, NikitaLogic logic) {
        ...
        btn.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                ...
                response.showform("webconnectionadd", request,"add", true);
                response.write();
            }
        });        
        ...
        nf.setOnActionResultListener(new NikitaForm.OnActionResultListener() {
            ...
            public void OnResult(NikitaRequest request, NikitaResponse response, Component component, String reqestcode, String responsecode, Nset result) {
                ...
                if(reqestcode.equals("delete") && responsecode.equals("button2")){
                    Nikitaset nikitaset = nikitaConnection.Query("SELECT connname FROM web_connection WHERE connid=?", result.getData("id").toString());
                    ...
                    nikitaset = nikitaConnection.Query("delete from web_connection where connid=?",result.getData("id").toString());
        ...
        comp.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                ...
                response.showform("webconnectionadd", request,"edit", true);
                response.write();
    ...
    private void fillGrid(NikitaResponse response){
        ...
        if(txt.getText().equals("")){
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"select connid,connname,connusername,connpassword,connclass,connurl,1 "+ 
                                                       "from web_connection "+(NikitaService.isModeCloud()?"WHERE createdby = '"+user+"' ":""));
            tablegrid.setData(nikiset);
        }
        else{
            String s = "%"+txt.getText()+"%";
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"select connid,connname,connusername,connpassword,connclass,connurl,1 "+ 
                                                       "from web_connection where "+(NikitaService.isModeCloud()?"createdby = '"+user+"' AND ":"")+"  connname like ? OR connclass like ? OR connurl like ? ; ",s,s,s);
            tablegrid.setData(nikiset);
        }
