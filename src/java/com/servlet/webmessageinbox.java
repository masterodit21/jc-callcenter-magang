 ...
 * @author user
 */
public class webmessageinbox extends NikitaServlet{
    ...
    public void OnCreate(NikitaRequest request, NikitaResponse response, NikitaLogic logic) {
        ...
        tablegrid.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                ...
                request.setParameter("mode","inbox");
                response.showform("webcomposemsg", request, "inbox", true);
                response.write();
            }
        });
    ...
    private void fillGrid(NikitaResponse response){
        ...
        if(txt.getText().equals("")){
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"SELECT messageid,sender,body,sdate,1 "+ 
                                                       "FROM nikita_message WHERE status = 'inbox' AND username = ? ORDER BY sdate DESC ",user);
            tablegrid.setData(nikiset);
        }
        else{
            String s = "%"+txt.getText()+"%";
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"SELECT messageid,sender,body,sdate,1 "+ 
                                                       "FROM nikita_message WHERE status = 'inbox' AND (sender LIKE ? OR body LIKE ?) AND username = ? ORDER BY sdate DESC ; ",s,s,user);
            tablegrid.setData(nikiset);
        }
