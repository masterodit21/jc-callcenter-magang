 ...
 * @author user
 */
public class webmoduleasset extends NikitaServlet{
    String user ;
    int dbCore;
    ...
    public void OnCreate(NikitaRequest request, NikitaResponse response, NikitaLogic logic) {
        ...
        txt2.setText(request.getParameter("assetname"));
        txt2.setTag(request.getParameter("assetid"));
        txt2.setStyle(new Style().setStyle("n-lock", "true"));
        nf.addComponent(txt2);
        ...
        btn.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {                
                ...
                response.showform("webnewasset", request,"new", true);  
                response.write();
            }
        });
        ...
        tablegrid.setDataHeader(Nset.readsplitString("|Name|assetfname|||")); //set header table
        style = new Style();
        ...
        nf.setOnActionResultListener(new NikitaForm.OnActionResultListener() {
            ...
            public void OnResult(NikitaRequest request, NikitaResponse response, Component component, String reqestcode, String responsecode, Nset result) {
                ...
                if(reqestcode.equals("new") && responsecode.equals("new-asset")){
                
                    for (int i = 0; i < result.getArraySize(); i++) {
                        Nikitaset nikitaset  = nikitaConnection.Query("INSERT INTO web_module_asset("
                                + "moduleid,assetid,createdby,createddate) VALUES(?,?,?,"+WebUtility.getDBDate(dbCore)+") ", response.getContent().findComponentbyId("modul-txtFinder").getTag(),result.getData(i).toString(),user);
                        
                        
                        nikitaset = nikitaConnection.Query("SELECT MAX(maid) FROM web_module_asset");
                        ...
                        data.setData("activitytype", "moduleasset");
                ...
                if(reqestcode.equals("asset") && responsecode.equals("OK")){
                ...
                if(reqestcode.equals("delete") && responsecode.equals("button2")){    
                    ...
                    Nikitaset nikitaset = nikitaConnection.Query("delete from web_module_asset where maid=?",result.getData("id").toString());
                    fillGrid(response);
                    response.writeContent();
                }
            }
        ...
        if(lblmode.getTag().equals("asset")){     
            nf.setText("Module Asset ["+nf.findComponentbyId("modul-txtFinder").getText()+"]"); 
    ...
    public void fillGrid(NikitaResponse response){
        ...
        if(txt.getText().equals("")){
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"select web_module_asset.maid,web_asset.assetname,web_asset.assetfname,web_module.moduleid,web_module.modulename,web_asset.assetid from web_module_asset LEFT JOIN web_module ON (web_module_asset.moduleid=web_module.moduleid)  LEFT JOIN web_asset ON (web_module_asset.assetid=web_asset.assetid)  WHERE web_module_asset.moduleid=?  ",txtFinder.getTag());
