 ...
 * @author user
 */
public class webmoduleresource extends NikitaServlet{
    String user ;
    int dbCore;
    ...
    public void OnCreate(NikitaRequest request, NikitaResponse response, NikitaLogic logic) {
        ...
        txt2.setText(request.getParameter("resname"));
        txt2.setTag(request.getParameter("resid"));
        txt2.setStyle(new Style().setStyle("n-lock", "true"));
        nf.addComponent(txt2);
        ...
        btn.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {                
                ...
                response.showform("webnewresources", request,"new", true);  
                response.write();
            }
        });
        ...
        tablegrid.setDataHeader(Nset.readsplitString("|Name|resfname|||")); //set header table
        style = new Style();
        ...
        nf.setOnActionResultListener(new NikitaForm.OnActionResultListener() {
            ...
            public void OnResult(NikitaRequest request, NikitaResponse response, Component component, String reqestcode, String responsecode, Nset result) {
                if(reqestcode.equals("new") && responsecode.equals("new-res")){
                
                    for (int i = 0; i < result.getArraySize(); i++) {
                        Nikitaset nikitaset = nikitaConnection.Query("SELECT resname FROM web_resource WHERE resid = ? ",result.getData(i).toString());
                        nikitaset  = nikitaConnection.Query("INSERT INTO web_module_resource("
                                + "moduleid,resid,resname,createdby,createddate) VALUES(?,?,?,?,"+WebUtility.getDBDate(dbCore)+
                                ") ", response.getContent().findComponentbyId("modul-txtFinder").getTag(),result.getData(i).toString(),nikitaset.getText(0, 0),user);
                        
                        
                        nikitaset = nikitaConnection.Query("SELECT MAX(mrid) FROM web_module_resource");
                        ...
                        data.setData("activitytype", "moduleresource");
                ...
                if(reqestcode.equals("resource") && responsecode.equals("OK")){
                    
                    response.getContent().findComponentbyId("modul-txtFinder2").setTag(result.getData("id").toString());
                    Nikitaset n = nikitaConnection.Query("SELECT formid FROM web_module_form WHERE moduleid=? AND formid=?", response.getContent().findComponentbyId("modul-txtFinder").getTag(), result.getData("id").toString());
                    if (n.getRows()==0) {
                        nikitaConnection.Query("INSERT INTO web_module_form (moduleid, formid,createdby,createddate) VALUES (?,?,?,"+WebUtility.getDBDate(dbCore)+")", response.getContent().findComponentbyId("modul-txtFinder").getTag(), result.getData("id").toString(),user);
                ...
                if(reqestcode.equals("delete") && responsecode.equals("button2")){    
                    ...
                    Nikitaset nikitaset = nikitaConnection.Query("delete from web_module_resource where mrid=?",result.getData("id").toString());
                    fillGrid(response);
                    response.writeContent();
                }
            }
        ...
        if(lblmode.getTag().equals("resource")){     
            nf.setText("Module Resource ["+nf.findComponentbyId("modul-txtFinder").getText()+"]"); 
    ...
    public void fillGrid(NikitaResponse response){
        ...
        if(txt.getText().equals("")){
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"select web_module_resource.mrid,web_resource.resname,web_resource.resfname,web_module.moduleid,web_module.modulename,web_resource.resid from web_module_resource LEFT JOIN web_module ON (web_module_resource.moduleid=web_module.moduleid)  LEFT JOIN web_resource ON (web_module_resource.resid=web_resource.resid)  WHERE web_module_resource.moduleid=?  ",txtFinder.getTag());
