 ...
 * @author user
 */
public class nikitatargetdevice extends NikitaServlet{
    ...
    */
    ...
    public void OnCreate(NikitaRequest request, NikitaResponse response, NikitaLogic logic) {
        ...
        nf.setText("Nikita Target Device");        
        ...
        txt.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                fillGrid(response);                  
                response.refreshComponent(response.getContent().findComponentbyId("target-tblTarget"));
            }
        }); 
                
        Component btn = new Image();
        btn.setId("target-btnAdd");
        ...
        btn.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                ...
                response.showform("nikitatargetdeviceadd", request,"add", true);
                response.write();
            }
        });        
        ...
        tablegrid.setId("target-tblTarget");
        tablegrid.setDataHeader(Nset.readsplitString("No|Account|Imei|"));
        ...
        tablegrid.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                fillGrid(response);
                if(response.getContent().findComponentbyId("target-btnAdd").getTag().equals("selected")){
                    Nset nset = Nset.newObject();
//                    nset.setData("targetaccount", "oi");
                    nset.setData("targetaccount", tablegrid.getData().getData(tablegrid.getSeletedRow()).getData(1).toString());
        ...
        nf.setOnActionResultListener(new NikitaForm.OnActionResultListener() {
            ...
            public void OnResult(NikitaRequest request, NikitaResponse response, Component component, String reqestcode, String responsecode, Nset result) {
                ...
                if(reqestcode.equals("delete") && responsecode.equals("button2")){
                    ...
                    nikitaConnection.Query("DELETE FROM nikita_target_device WHERE targetid=?",result.getData("id").toString());
                    fillGrid(response);
                    response.writeContent();
                }
            }
        ...
        comp.setOnClickListener(new Component.OnClickListener() {
            ...
            public void OnClick(NikitaRequest request, NikitaResponse response, Component component) {
                ...
                response.showform("nikitatargetdeviceadd", request,"edit", true);
                response.write();
    ...
    private void fillGrid(NikitaResponse response){
        Tablegrid tablegrid = (Tablegrid)response.getContent().findComponentbyId("target-tblTarget") ;
        Textbox txt= (Textbox)response.getContent().findComponentbyId("target-txtSearch");
        Component cmpBtnAdd =response.getContent().findComponentbyId("target-btnAdd") ;
        
                
        NikitaConnection nikitaConnection = response.getConnection("logic");
       
        if(txt.getText().equals("")){
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"SELECT targetid,targetaccount,targetimei,1 "+ 
                                                       "FROM nikita_target_device");
            tablegrid.setData(nikiset);
        }
        else{
            String s = "%"+txt.getText()+"%";
            Nikitaset nikiset = nikitaConnection.QueryPage(tablegrid.getCurrentPage(),tablegrid.getShowPerPage(),"SELECT targetid,targetaccount,targetimei,1 "+ 
                                                       "FROM nikita_target_device WHERE targetaccount LIKE ? OR targetimei LIKE ? ; ",s,s);
            tablegrid.setData(nikiset);
        }
        
        if(cmpBtnAdd.getTag().equals("selected")){
            tablegrid.setColHide(3, true);
            cmpBtnAdd.setVisible(false);
        }
        ...
        tablegrid.setAdapterListener(new IAdapterListener() {
            ...
            public Component getViewItem(int row, int col, Component parent, Nset data) {
                if(col == 3){
                    ...
                    btn = new Image();
                    btn.setId("target-btnDelete["+row+"]");
